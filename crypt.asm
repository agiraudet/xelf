...
_start:
  ...
  key: db 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46
...
decrypt_init:
  ...
  lea edi, [rel key]           ; Load the address of the key

decrypt_loop:
    mov al, [rsi]            ; Load a byte from the ciphertext
    xor al, [edi]            ; XOR with the corresponding byte from the key
    mov [esi], al            ; Store the result back in the ciphertext
    inc esi                  ; Move to the next byte in the ciphertext
    inc edi                  ; Move to the next byte in the key
